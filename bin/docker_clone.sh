...
if [ "$DOCKER_BUILD" = "build" ]; then
    ...
    if [ "$PG_VERSION_NUM" -ge 90500 ]; then git clone -b main https://github.com/RekGRpth/pg_statement_rollback.git; fi
    ...
    if [ "$PG_VERSION_NUM" -ge 120000 ]; then git clone -b main https://github.com/RekGRpth/pg_statement_rollback.git; fi
